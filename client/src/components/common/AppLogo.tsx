import React from 'react'
import Link from './Link'


type appLogo = {
     title?: string,
     fontSize?: string,
     showTitle?: boolean,
     className?: string,
}


const AppLogo = ({ title = "", fontSize = "text-2xl", showTitle = true, className }: appLogo) => {


     switch (fontSize) {
          case "xs":
               fontSize = "text-xs"
               break;
          case "sm":
               fontSize = "text-sm"
               break;
          case "md":
               fontSize = "text-md"
               break;
          case "lg":
               fontSize = "text-lg"
               break;
          case "xl":
               fontSize = "text-xl"
               break;

          default:
               fontSize = fontSize;
     }

     return (
          <Link href='/' className='flex items-center mx-3 w-fit'>
               <svg viewBox="0 0 32 32" height="24" xmlns="http://www.w3.org/2000/svg" focusable="false" aria-hidden="true">
                    <defs>
                         <linearGradient x1="94.092%" x2="56.535%" y1="6.033%" y2="43.087%" id="uid47">
                              <stop stopColor="#0052CC" offset="18%"></stop>
                              <stop stopColor="#2684FF" offset="100%"></stop>
                         </linearGradient>
                    </defs>
                    <g stroke="none" strokeWidth="1" fillRule="nonzero">
                         <path fill="#2684FF" d="M26.0406546,5 L14.9983562,5 C14.9983562,6.32163748 15.5233746,7.58914413 16.4579134,8.52368295 C17.3924523,9.45822178 18.6599589,9.98324022 19.9815964,9.98324022 L22.0151159,9.98324022 L22.0151159,11.9465283 C22.0168782,14.6974491 24.2474348,16.9265768 26.9983562,16.9265762 L26.9983562,5.95770152 C26.9983562,5.42877757 26.5695786,5 26.0406546,5 Z"></path>
                         <path fill="url(#uid47)" d="M20.0420436,11 L9,11 C9.00176139,13.7504065 11.2309666,15.9796117 13.9813731,15.9813731 L16.0154337,15.9813731 L16.0154337,17.9451836 C16.0154337,19.2671728 16.5405919,20.5350167 17.4753794,21.4698042 C18.4101669,22.4045917 19.6780108,22.9297499 21,22.9297499 L21,11.9579564 C21,11.4288917 20.5711083,11 20.0420436,11 Z"></path>
                         <path fill="url(#uid47)" d="M14.0420436,17 L3,17 C3.00176275,19.7516528 5.23291286,21.9813736 7.98456626,21.9813731 L10.0250133,21.9813731 L10.0250133,23.9451836 C10.0250082,26.6943468 12.2508419,28.9244664 15,28.9297499 L15,17.9579564 C15,17.4288917 14.5711083,17 14.0420436,17 Z"></path>
                    </g>
               </svg>
               {showTitle && <div className={`text-[#333333] font-normal mx-3 ${fontSize} ${className}`}>{title}</div>}
          </Link>
     )
}

export default AppLogo